...
- Run `cargo fmt -- --check` to verify code formatting (CI will fail if not formatted)
- Run `cargo clippy -- -D warnings` to check for linting issues (CI will fail if there are warnings)

### Rust Guidelines
- Use idiomatic Rust patterns: iterators, pattern matching, `?` operator for error propagation
- Avoid adding new `unsafe` blocks; existing `unsafe` code is confined to FFI operations in `src/lib.rs`
- Document all public APIs with Rust doc comments (`///` for items, `//!` for modules)
- Match the error handling style in `src/lib.rs`: use `Result` with `InitializeError` variants

### FFI and Safety
- The DLT C library bindings are auto-generated by `build.rs` using `bindgen` into `OUT_DIR/libdlt_bindings.rs`
- `src/libdlt.rs` is a hand-written wrapper that includes the generated bindings via `include!`; do not edit it manually
- `unsafe impl Send` and `unsafe impl Sync` on `DltLogger` are intentional — the DLT library is thread-safe by design

### Test Coverage
- 100% code coverage is enforced in CI; all new code paths must be covered by tests
- Unit tests live in `src/lib.rs` (in `#[cfg(test)]` module); integration tests live in `tests/`
- Integration tests interacting with the DLT daemon live in `tests/` and require `dlt-daemon` to be running
...
│   ├── libdlt.rs         # Wrapper that includes auto-generated FFI bindings via include!()
